...
original_author="darkcode357"
original_name="flask_model"
original_urlname="flask-model"
original_description="Awesome flask_model created by darkcode357" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_model "${name}"
...
then
    ...
    echo "then run 'flask_model' or 'python -m flask_model'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_model' or 'python -m flask_model' or 'poetry run flask_model'"
fi
