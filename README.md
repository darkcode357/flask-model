...
- 🎯 Entry points to execute your program using `python -m <flask_model>` or `$ flask_model` with basic CLI argument parsing.
...
---
# flask_model Flask Application

Awesome flask_model created by darkcode357
...
```bash
git clone https://github.com/darkcode357/flask-model flask_model
cd flask_model
make install
```
...
```bash
flask_model create-db   # run once
flask_model populate-db  # run once (optional)
flask_model add-user -u admin -p 1234  # ads a user
flask_model run
```
